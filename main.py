import asyncio, logging, os, secrets, string, time, html
import db, filter
...
async def sync_all_users_from_panel(auth: AuthManager):
    ...
    for c in clients:
        ...
        db.update_user_from_panel(tg_id, c_date, p_until, group, email)
...
async def background_payment_check(auth: AuthManager):
    ...
    while True:
        try:
            ...
            users = db.get_users_for_payment_check()
            now = int(time.time())
            ...
            logger.error(f"Error processing background_payment_check: {e}")
...
async def status_cmd(message: types.Message, auth: AuthManager):
    ...
    try:
        ...
        paid, expires, expired = db.get_users_by_payment_status()
...
async def send_payment_message(message: types.Message, state: FSMContext):
    _, status_0, status_minus_1 = db.get_users_by_payment_status()
...
async def add_vpn_client(user_info, auth: AuthManager, target_inbounds: list[int]):
    ...
    result = await auth.api_request("POST", "/panel/api/clients/add", json=payload)
    log_message = result.get("msg", "API error")

    if result.get("success"):
        logger.info(f"Created new VPN client: {user_email} (inbounds: {target_inbounds})")
        db.update_user_email(tg_id, user_email)
        return sub_id, "Created successfully"

    if "email already in use" in log_message.lower():
        existing_client = await get_client_by_email(user_email, auth)
        if existing_client and existing_client.get("subId"):
            db.update_user_email(tg_id, user_email)
            return existing_client.get("subId"), "Client already exists"

    logger.error(f"Error creating new VPN client {user_email}: {log_message}")
...
async def resolve_existing_client(user_info, auth: AuthManager):
    ...
    if saved_email:
        client = await get_client_by_email(saved_email, auth)
        if client:
            return client

    legacy_emails = get_user_emails(user_info)
    for candidate_email in legacy_emails:
        client = await get_client_by_email(candidate_email, auth)
        if client:
            actual_email = client.get("email") or candidate_email
            db.update_user_email(tg_id, actual_email)
            logger.info(f"Email '{actual_email}' is fixed in database for user with tg_id={tg_id}")
            return client
    return None
...
async def get_client_credentials(user_info, auth: AuthManager):
    ...
    sub_id, msg = await add_vpn_client(user_info, auth, target_inbounds)
    if not sub_id:
        raise Exception(msg)
    return sub_id
...
async def create_token(message: types.Message, auth: AuthManager):
    ...
    try:
        sub_id = await get_client_credentials(message.from_user, auth)

        sub_url_base = Config.SUB_URL
...
async def help_cmd(message: types.Message):
    if db.is_user_approved(message.from_user.id) < 1:
        await message.answer(Config.access_denied_error)
        return
...
async def main():
    ...
    await auth.check_connection()

    dp.message.middleware(filter.BannedUserMiddleware())
    dp.callback_query.middleware(filter.BannedUserMiddleware())
    asyncio.create_task(background_payment_check(auth))

    try:
        await dp.start_polling(bot, auth=auth)
    finally:
